...
def good_AP_finder(time,voltage):
    ...
    ##Your Code Here!
    tolarence = 50
    a = time[voltage>max(voltage)-tolarence]
    b = time[voltage<min(voltage)+tolarence]
    index = np.concatenate((a,b))
    index = np.around(index,3)
    APTime = np.sort(index)
    APTimes = np.unique(APTime)
    return APTimes
...
def plot_spikes(time,voltage,APTimes,titlestr):
    ...
    ##Your Code Here
    p = np.ones(len(APTimes)) + max(voltage) + 20
...
#    APTime = bad_AP_finder(t,v)
    ...
    plot_spikes(t,v,APTime,'Action potentials in raw signals')
