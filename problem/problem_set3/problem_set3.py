...
def get_ems(df, trial):
    ...
    t = np.where((df['em_time'][trial] >= df['stimon'][trial]) & (df['em_time'][trial] <= df['response'][trial]))
    t = t[0][:]
    h = df['em_horiz'][trial][t]
    v = df['em_vert'][trial][t]
...
def plot_ems_and_target(df, trial):
    ...
    target_x = np.repeat(df['targ_x'][trial], len(t))
    target_y = np.repeat(df['targ_y'][trial], len(t))

    plt.figure()

    # *** YOUR CODE HERE ***
    plt.plot(t,h,'r',t,v,'g',t,target_x,'r',t,target_y,'g')
    plt.ylim(-10,10)
    plt.legend(('Horizontal','Vertical'),loc=4)
    plt.title("Eye movements for trial %d" % trial)
    plt.xlabel('Time(ms)')
    plt.ylabel('Positions (Deg in visual angle)')
    plt.show()
...
def get_rate(spk_times, start, stop):
    ...
    rate = spk_times[(spk_times>start) & (spk_times<stop)]
    rate = rate.size/(stop-start)
...
if __name__ == "__main__":
    df = load_data()
    add_info(df)
    result = rts_by_targ_ecc(df)
    plot_rts(df)
    plot_ems_and_target(df,213)
    
    add_aligned_rates(df, 'stimon' ,100, 200)
    responseVSposition = df.pivot_table(values='rates_stimon_100_200', index='targ',columns='targ_ecc')
    responseVSposition.plot(kind='bar')    
    plt.title('Image type vs position')
    plt.ylabel('Spike rate')    
    
    add_aligned_rates(df, 'targ_acq' ,100, 200)
    responseVStime = df.pivot_table(values='rates_targ_acq_100_200', index='targ',columns='targ_ecc')
    responseVStime.plot(kind='bar')
    plt.title('Image type vs time taken')
    plt.ylabel('reaction times')
